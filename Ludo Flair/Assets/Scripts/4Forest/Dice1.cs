...
public class Dice1 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (wonYet(GameMovement1.colNumber) == true)
        {
            needPiece = false;
            //diceEngaged = false;
            GameMovement1.colNumber = nextColor(GameMovement1.colNumber);
        }
        ...
        if (needPiece == true)
        {
            ...
            pieceSelection = pieceDecider(GameMovement1.colNumber, finalSide);
        ...
        if (Input.GetMouseButtonDown(0)) // If Click Registered
        {
            ...
            if (hit.collider != null)// Checks If An Object Was Hit
            {
                ...
                if (hit.collider.gameObject.tag == "Piece") // checks if the object hit is a piece
                {
                    ...
                    int choosenColour = hit.collider.gameObject.GetComponent<Waypoints1>().pieceColour;
                    int choosenPiece = hit.collider.gameObject.GetComponent<Waypoints1>().pieceNumber;
    ...
    {
        if (diceEngaged == false) //& !Waypoints1.placingToken) // Does not Allow you to play if A turn Is In Motion
        {
            StartCoroutine("RollTheDice");
            Waypoints1.placingBomb = false;
            Waypoints1.placingCurse = false;
            ...
            for(int c = 0; c <4; c++)
            {
                for(int i= 0; i < 4; i++)
                {
                    ...
                    //{
                    checker += "/" + (GameMovement1.pos_index_golbal[c, i]).ToString() + "/";
                    //}
                }
            }
            Debug.Log(checker);*/
    ...
    // Stores the unique codes of all pieces that the player can move
    int[] moveablePiecesArray; // piece code = piece colour *10 + piece number 


    int[] collisionPrediction(int curCol, int pieceNumber, int diceNumber)// (0, 0, num)
    {
        int[] collisionsArray = new int[] { };

        int predictedLocation = modulo52(GameMovement1.pos_index_golbal[curCol, pieceNumber] + diceNumber);
        ...
        bool eatingPosibile = ((GameMovement1.pos_index[curCol, pieceNumber] + diceNumber < 50) & (sixCondition[curCol, pieceNumber] == true) & (!isSafeSpot));
        ...
        if (eatingPosibile)
        {
            for (int c = 0; c < 4; c++)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (predictedLocation == GameMovement1.pos_index_golbal[c, i])
                    {
    ...
    bool eligibleToMove(int curCol, int diceNumber)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if ((GameMovement1.pos_index[curCol, i] + diceNumber < 58) & (sixCondition[curCol, i] == true))
            {
        ...
        int status = Waypoints1.curseStatus[curCol];

        // Checking If Cursed And If So Skipping Turn Accordingly    
        if ((status == 1) & ((diceNumber%2) == 1) || (status == 2) & ((diceNumber % 2) == 0))
        {
            return false;
        }
    ...
    private IEnumerator RollTheDice()
    {
        ...
        if (eligibleToMove(GameMovement1.colNumber, finalSide) == false) // If Player Is Not Eligible to Move 
        {
        ...
        else
        {
            ...
            {
                pieceSelection = moveablePiecesArray[0] - (10 * GameMovement1.colNumber);
                StartCoroutine("MoveThePieces");
            }
            else
            {
    ...
    private IEnumerator MoveThePieces()
    {
        ...
        int localColNum = GameMovement1.colNumber;
        bool isSix = (finalSide == 6);
        ...
        // Did The Piece Reach?
        if (GameMovement1.pos_index[localColNum, pieceSelection] + finalSide == 57)
        {
            finishCondition[localColNum, pieceSelection] = true;
            justReached = true;
        }

        // has the player eaten a piece
        bool hasEaten = false;
        ...
        if (isSix)
        {
            if (sixCondition[localColNum, pieceSelection] == false)
            {
                ...
                GameMovement1.pos_index[localColNum, pieceSelection]++;
            }
            else
            {
        ...
        if (collision.Length == 1)
        {
            ...
            while (GameMovement1.pos_index[pieceColour, pieceNumber] != 0)
            {
                GameMovement1.pos_index[pieceColour, pieceNumber]--;
                yield return new WaitForSeconds(0.15f);
            }
        }


        //Debug.Log(GameMovement.pos_index_golbal);
        ...
        for (int c = 0; c < 4; c++)
        {
            for (int i = 0; i < 4; i++)
            {
                checker += "/" + (GameMovement.pos_index_golbal[c, i]).ToString() + "/";
            }
        }
        Debug.Log(checker);*/
        ...
        {
            GameMovement1.colNumber = nextColor(localColNum);
            diceEngaged = false;
        }
    }
}
